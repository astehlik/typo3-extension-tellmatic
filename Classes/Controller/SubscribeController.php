...
class SubscribeController extends ActionController
{
    ...
    public function subscribeConfirmAction($authCode)
    {
        try {
            ...
            $subscriptionHandler->handleSubscribeConfirmation(
                $authCode->getIdentifier(),
                'Subscription confirmed by auth code in ' . __METHOD__
            );
    ...
    public function subscribeRequestFormAction()
    {
        ...
        if ($this->controllerContext->getRequest()->getOriginalRequest() !== null
            ...
            $this->view->assign(
                'acceptConditions',
                $this->controllerContext->getRequest()->getOriginalRequest()->getArgument('acceptConditions')
            );
        }
    }
    ...
    public function unsubscribeAction(
        ...
        try {
            ...
            $subscriptionHandler = $this->getSubscriptionHandler();
            $subscriptionHandler->handleUnsubscribeSubmit(
                $authCode->getIdentifier(),
                $historyId,
                $queueId,
                $newsletterId,
                'Unsubscribed by auth code in ' . __METHOD__
            );
     ...
     * @validate $additionalData Sto\Tellmatic\Validation\AdditionalFieldValidator
     */
    public function updateAction($email, $authCode, array $additionalData = [])
    {
        try {
            $authCode = $this->getValidAuthCode($authCode);
            ...
            if ($email !== $authCode->getIdentifier()) {
                ...
                $subscriptionHandler->handleUnsubscribeSubmit(
                    ...
                    0,
                    0,
                    'Unsubscribed because of email address change in ' . __METHOD__
                );
                $subscriptionHandler->handleSubscribeRequest(
                    $email,
                    $additionalData,
                    'New subscription because of email address change in ' . __METHOD__
                );
            } else {
                $subscriptionHandler = $this->getSubscriptionHandler();
                $subscriptionHandler->handleUpdateSubmit(
                    $authCode->getIdentifier(),
                    $additionalData,
                    'Data updated by auth code in ' . __METHOD__
                );
            }
    ...
    public function updateFormAction($authCode)
    {
        try {
            ...
            if ($subscribeStateResponse->getSubscribeState()
                === SubscribeStateResponse::SUBSCRIBE_STATE_NOT_SUBSCRIBED
            ) {
            ...
            if (!empty($this->settings['additionalFields']) && is_array($this->settings['additionalFields'])) {
                ...
                foreach ($this->settings['additionalFields'] as $fieldName => &$fieldSettings) {
                    if (!empty($addressData[$fieldName])) {
                        $fieldSettings['value'] = $this->getSubmittedValueOrDefault(
    ...
    {
        $authCodeObject = $this->authCodeRepository->findOneIndependentByAuthCodeAndContext(
            $authCode,
            static::AUTH_CODE_CONTEXT
        );
        if (!isset($authCodeObject)) {
            $authCodeObject = $this->authCodeRepository->findOneIndependentByAuthCodeAndContext(
                $authCode,
                AuthCodeCommandController::AUTH_CODE_CONTEXT
            );
        }
        if (!isset($authCodeObject)) {
    ...
    protected function handleException(\Exception $exception)
    {
        if (isset($this->logger)) {
            $this->logger->critical(
                'An error has occured during tellmatic subscription: ' . $exception->getMessage()
                . LF . $exception->getTraceAsString()
            );
        }
        ...
        if ($this->request instanceof \TYPO3\CMS\Extbase\Mvc\Web\Request
            ...
            if ($referringRequest->getControllerActionName() !== $this->request->getControllerActionName()
